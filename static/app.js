...
async function load() {
  ...
  try {
    ...
    statusEl.textContent = c
      ...
      : "Can't reach the server. Is app.py running?";
    if (!c) app.innerHTML =
      '<div class="boot">No saved data yet. Start app.py and refresh.</div>';
  }
}
...
btn.addEventListener("click", async () => {
  ...
  statusEl.textContent = "Querying live fares — this takes a few seconds";
  try {
    ...
    render(data);
    store(data);
    statusEl.textContent = data.warning || "Updated";
  } catch (e) {
    statusEl.textContent = String(e.message || e).includes("already")
      ? "A check is already running"
      : "Check failed — Google may have changed format. See farewatch.log";
  } finally {
    btn.disabled = false;
    btn.textContent = "Check fares now";
  }
});
